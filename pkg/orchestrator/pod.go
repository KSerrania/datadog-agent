...
func ProcessPodList(podList []*v1.Pod, groupID int32, hostName string, clusterID string, cfg *config.OrchestratorConfig) ([]model.MessageBody, error) {
	...
	for _, p := range podList {
		...
		// We need stdlib-style marshalling to ensure consistent map ordering
		...
		hash := murmur3.Sum64(jsonPodModel)
		customResourceVersion := fmt.Sprint(hash)
		log.Debugf("Custom resource version is %s for pod model %s", customResourceVersion, jsonPodModel)

		if SkipKubernetesResource(p.UID, customResourceVersion, K8sPod) {
			continue
		}

		// Replace the payload metadata field with the custom version.
		podModel.Metadata.ResourceVersion = customResourceVersion
